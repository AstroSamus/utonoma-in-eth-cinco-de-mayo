import { createStateForDislikeButton } from './DislikeButton.state.js'
...
export const DislikeButton = () => { 
  const state = createStateForDislikeButton()

  const $buttonDislikeShortVideo = document.querySelector('#buttonDislikeShortVideo')
    ...
    switch (state.currentStep()) {
      ...
      case state.availiableSteps.waitingForApproveOnWallet:
        try {
          ...
          likeResult = await utonomaContractForSignedTransactions.dislike([state.utonomaIdentifier().index, state.utonomaIdentifier().contentType])
